...
%函数返回值imf为本征模态向量,ort=一个数，nbits为1*4的向量？
...
%---------------------------------------------------------------------------------------------------
% 计算包络线们和振幅估计模式的平均值
%INIERP为插值函数，MODE_COMPLEX复杂模式，ndirs无本金交割利率互换？
...
%---------------------------------------------------------------------------------------
% defines new extrema points to extend the interpolations at the edges of the定义了新的极值点来扩展插入的边缘
...
%---------------------------------------------------------------------------------------------------
%extracts the indices of extrema提取的极值指数
...
%当两个或两个以上的连续点有相同的值我们认为只有一个极值的常数
